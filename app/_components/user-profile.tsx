    ...
    <article className="bg-dune relative mx-auto flex w-full max-w-4xl flex-col rounded-md p-6 md:flex-row md:gap-6 md:p-12 md:pb-0">
      <aside className="relative z-5 flex flex-1 flex-col space-y-6 rounded-t-md bg-zinc-600/50 p-6 pt-20">
        <header className="absolute top-0 left-0 h-40 w-full">
          ...
          <div className="group absolute top-2 right-2 rounded-md border border-zinc-600/50 bg-zinc-600/50 px-2 py-1.5 backdrop-blur-sm">
            <CopyLink
...
function SocialLink({ profile, platform }: { profile: FullProfile; platform: string }) {
  ...
  return (
    <a
      ...
      aria-label={
        isEmail
          ? `Send email to ${profile.display_name || profile.username}`
          : `Visit ${profile.display_name || profile.username}'s ${link.platform} profile`
      }
    >
