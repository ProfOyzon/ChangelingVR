...
import { Pagination } from '@/app/teams/components/pagination';
...
async function PaginationWrapper({ query }: { query: string }) {
  ...
  return <Pagination totalPages={pages} />;
}

export default async function Page(props: PageProps<'/teams'>) {
  const { query = '', page = '1' } = (await props.searchParams) as { query: string; page: string };

  return (
    <div className="mx-auto my-4 flex min-h-[calc(100svh-4rem)] flex-col items-center gap-6 max-md:p-6">
